...
- current implemented status:
  ...
  - Electricity `dailyTrend` and `areaComparison` now own their own `height.view / pdf` branches instead of relying on a shared chart-height bucket
  - Electricity `areaShare` now exposes object-local donut config under `report.section.electric.chart.areaShare.*`, including `legend`, `pie`, `centerGraphic`, and `height`; daily PDF no longer relies on one catch-all `.pdf-daily-chart-grid .electricity-chart` height override for these cards
  ...
  - the live `config/report_style.json` no longer keeps duplicated top-level legacy branches such as `components.summaryCard`, `components.table`, `components.reportContainer`, `components.reportTitle`, `components.reportSubtitle`, `components.reportMetadata`, `components.reportHeader`, or `components.footer`; the loader now expects the canonical `components.report.*` tree, with only `pdfBase` / `pdfCompact` height collapse retained as a compatibility shim inside the report tree itself
  - header/title/subtitle/metadata CSS now reads direct `components.report.titleHeader.*` variables rather than the older alias layer, which reduced the compatibility alias surface again
  - the active CSS assets now also read colors, text, borders, shadows, radii, and spacing directly from canonical report tokens, so the old generic alias bridge variables such as `--bg-page`, `--text-main`, `--border`, `--space-md`, and `--shadow-card` are no longer emitted by `ReportStyleService`
