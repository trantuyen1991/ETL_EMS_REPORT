...
\begin{table}[h]
	...
	\begin{DocHybridTable}{|>{\raggedright\arraybackslash}p{3.0cm}|>{\raggedright\arraybackslash}p{3.4cm}|>{\raggedright\arraybackslash}X|}
...
\begin{table}[h]
	...
	\begin{DocHybridTable}{|>{\raggedright\arraybackslash}p{3.2cm}|>{\raggedright\arraybackslash}p{3.4cm}|>{\raggedright\arraybackslash}X|}
		\textbf{Artifact / Folder} & \textbf{Canonical Form} & \textbf{Meaning in this project} \\ \hline
		\texttt{view\_html} & \texttt{output/reports/YYYY\_MM/view\_html/} & Browser-readable HTML artifact dùng để đọc report, rà layout logic, và debug content structure ngoài print path. \\ \hline
		\texttt{pdf\_source\_html} & \texttt{output/reports/YYYY\_MM/pdf\_source\_html/} & HTML nguồn dùng cho PDF export, phản ánh print-safe structure gần nhất trước khi gọi Chromium/CDP print. \\ \hline
		\texttt{pdf} & \texttt{output/reports/YYYY\_MM/pdf/} & PDF artifact cuối cùng dùng cho release review, chia sẻ nội bộ, và lưu trữ theo kỳ báo cáo. \\ \hline
		\texttt{excel} & \texttt{output/reports/YYYY\_MM/excel/} & Folder chứa workbook Excel daily-only ở scope export hiện tại. \\ \hline
		\texttt{docs/output/} & documentation output & Nơi chứa generated technical-documentation PDF của subsystem LaTeX; các file PDF ở đây là build artifacts, không phải source tài liệu. \\ \hline
		\texttt{\_legacy\_backup\_2026\_05\_11/} & recoverable backup folder & Backup nội bộ dùng để di chuyển legacy output artifacts ra khỏi canonical root mà không xóa hẳn. \\ \hline
	\end{DocHybridTable}
\end{table}
...
\begin{table}[h]
	...
	\begin{DocHybridTable}{|>{\raggedright\arraybackslash}p{3.2cm}|>{\raggedright\arraybackslash}p{3.1cm}|>{\raggedright\arraybackslash}X|}
