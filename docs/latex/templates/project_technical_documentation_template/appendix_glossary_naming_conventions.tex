...
\begin{table}[htbp]
	...
	\begin{tabularx}{\linewidth}{|>{\RaggedRight\arraybackslash}p{3.0cm}|>{\RaggedRight\arraybackslash}p{3.4cm}|>{\RaggedRight\arraybackslash}X|}
		...
		\texttt{total\_energy} & database / business source & View nguồn chuẩn cho official plant totals và official area totals của Electricity section. \\ \hline
		\texttt{energy\_kpi} & database / business source & Table nguồn chuẩn cho KPI reporting, production-linked values, và các KPI rows đã được business chấp nhận. \\ \hline
		...
		Source-of-truth & business rules & Khái niệm chỉ nguồn dữ liệu được coi là authoritative cho một metric hoặc report block. Với Electricity official totals, source-of-truth hiện tại là \texttt{total\_energy}; với KPI, source-of-truth là \texttt{energy\_kpi}. \\ \hline
...
\begin{table}[htbp]
	...
	\begin{tabularx}{\linewidth}{|>{\RaggedRight\arraybackslash}p{3.2cm}|>{\RaggedRight\arraybackslash}p{3.4cm}|>{\RaggedRight\arraybackslash}X|}
		...
		\texttt{view\_html} & \TablePath{OUTPUT_DIR/YYYY_MM/view_html/} & Browser-readable HTML artifact dùng để đọc report, rà layout logic, và debug content structure ngoài print path. \\ \hline
		\texttt{pdf\_source\_html} & \TablePath{OUTPUT_DIR/YYYY_MM/pdf_source_html/} & HTML nguồn dùng cho PDF export, phản ánh print-safe structure gần nhất trước khi gọi Chromium/CDP print. \\ \hline
		\texttt{pdf} & \TablePath{OUTPUT_DIR/YYYY_MM/pdf/} & PDF artifact cuối cùng dùng cho release review, chia sẻ nội bộ, và lưu trữ theo kỳ báo cáo. \\ \hline
		\texttt{excel} & \TablePath{OUTPUT_DIR/YYYY_MM/excel/} & Folder chứa workbook Excel daily-only ở scope export hiện tại. \\ \hline
		\TablePath{OUTPUT_DIR/_staging/} & temporary print staging & Nơi giữ staging HTML/PDF cho Chromium print path; runtime sẽ tự dọn current-batch staging files và prune thư mục này nếu rỗng. \\ \hline
...
Output canonical của hệ thống hiện đi theo pattern \texttt{OUTPUT\_DIR/YYYY\_MM/}. Nếu \texttt{OUTPUT\_DIR} để trống, runtime mới fallback về \texttt{project\_root/output/reports/YYYY\_MM/}. Điều này có nghĩa là:
...
\item artifact family được tách theo subfolder thay vì trộn tất cả file vào root của \texttt{OUTPUT\_DIR/};
\item \texttt{\_staging/} chỉ là temporary print folder, không phải reader-facing final output tree.
\end{enumerate}
...
\begin{enumerate}[label=-]
\item \texttt{energy\_kpi}: database table authoritative cho KPI và production-linked values, không phải tên một chart hay section title.
\item \texttt{total\_energy}: database view authoritative cho Electricity official totals, không phải bảng meter detail.
...
\item \texttt{/srv/energy-report-output/2025\_06/pdf/}: ví dụ canonical folder cho PDF artifacts tháng 2025-06, không phải staging directory.
\end{enumerate}
