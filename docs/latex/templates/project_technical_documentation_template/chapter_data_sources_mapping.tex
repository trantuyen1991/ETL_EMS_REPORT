...
\item \texttt{total\_energy}
\item \texttt{utility\_usage}
\item \texttt{energy\_kpi}
\end{enumerate}

Trong số này, \texttt{diode\_energy}, \texttt{ico\_energy}, và \texttt{sakari\_energy} là các view phục vụ electricity detail và comparison theo area. \texttt{total\_energy} là view source-of-truth cho Electricity official totals. \texttt{utility\_usage} là view phục vụ Utility reporting. \texttt{energy\_kpi} là table đặc biệt vì nó mang vai trò source-of-truth cho KPI reporting và production-linked reporting.

Một lưu ý vận hành quan trọng là \texttt{total\_energy} không phải lúc nào cũng bảo đảm có row cho mọi ngày lịch. Trong thực tế có thể xuất hiện ngày không sản xuất, ngày dừng vận hành, hoặc ngày pipeline nguồn không ghi row vào view này. Vì vậy downstream code không được giả định rằng mọi ngày dense calendar đều có sẵn source row ở \texttt{total\_energy}.
...
\begin{tabularx}{\linewidth}{|>{\RaggedRight\arraybackslash}p{2.6cm}|>{\RaggedRight\arraybackslash}p{2.1cm}|>{\RaggedRight\arraybackslash}p{2.9cm}|>{\RaggedRight\arraybackslash}p{1.7cm}|>{\RaggedRight\arraybackslash}X|}
		\hline
		\textbf{Logical Dataset} & \textbf{Object Type} & \textbf{Physical Object} & \textbf{Date Column} & \textbf{Primary Use} \\ \hline
		\texttt{all\_energy} & view & \texttt{all\_energy\_extend} & \texttt{dt} & nguồn tổng quát cho energy-related exploration hoặc future aggregation slices. \\ \hline
		\texttt{diode\_energy} & view & \texttt{diode\_energy\_extend} & \texttt{dt} & chi tiết điện năng theo area DIODE. \\ \hline
		\texttt{ico\_energy} & view & \texttt{ico\_energy\_extend} & \texttt{dt} & chi tiết điện năng theo area ICO. \\ \hline
		\texttt{sakari\_energy} & view & \texttt{sakari\_energy} & \texttt{dt} & chi tiết điện năng theo area SAKARI. \\ \hline
		\texttt{total\_energy} & view & \texttt{total\_energy} & \texttt{dt} & official plant totals và official area totals cho Electricity section. \\ \hline
		\texttt{utility\_usage} & view & \texttt{utility\_usage} & \texttt{dt} & utility daily detail, utility summary, và utility comparison blocks. \\ \hline
		\texttt{energy\_kpi} & table & \texttt{energy\_kpi} & none at config level & source-of-truth cho KPI rows, production values, và energy-linked KPI reporting. \\ \hline
		\texttt{processvalue} & raw table & \texttt{processvalue} & \texttt{Time\_Stamp} & raw utility sensor rows cho sensor monitoring, anomaly-ready aggregation, và trend context. \\ \hline
	\end{tabularx}
\end{table}

\subsection{Energy KPI as authoritative source}

\texttt{energy\_kpi} vẫn là nguồn quan trọng về mặt nghiệp vụ cho KPI và production-linked reporting. Table này chứa KPI values, production context, và các stored blocks cần cho KPI coverage logic. Trong khi đó, \texttt{total\_energy} là view chuyên trách cho Electricity official totals. Chính vì vậy:

\begin{enumerate}[label=-]
\item Electricity official totals phải dựa trên \texttt{total\_energy};
...
\begin{table}[htbp]
	...
	\begin{tabularx}{\linewidth}{|>{\RaggedRight\arraybackslash}p{3.0cm}|>{\RaggedRight\arraybackslash}p{3.3cm}|>{\RaggedRight\arraybackslash}X|}
		...
		\texttt{diode\_energy}, \texttt{ico\_energy}, \texttt{sakari\_energy}, \texttt{total\_energy}, \texttt{utility\_usage}, \texttt{energy\_kpi} & \texttt{EnergyDataRepository} & fetch daily rows, inspect columns, expose meter columns, and provide structured tabular data for higher-level services. \\ \hline
...
\texttt{EnergyService} chịu trách nhiệm dựng domain object cho Electricity. Nó nhận area rows, area columns, và official-total inputs từ \texttt{total\_energy}, rồi trả ra các nhóm dữ liệu như:
...
Một chi tiết quan trọng là \texttt{EnergyService} không xem raw area views là official totals. Nó sử dụng \texttt{total\_energy} làm reference cho plant total và area total theo các rule đã chốt. Plant total trong report đọc trực tiếp từ \texttt{total\_energy.Total\_engy}; backend không cộng thêm \texttt{SAKARI\_engy} để tự suy ra plant total.

Bên cạnh đó, lớp Electricity phải xử lý được trường hợp \texttt{total\_energy} bị sparse theo ngày. Dense calendar ở report layer vẫn phải được giữ nguyên, nhưng các bước arithmetic-only như snapshot tổng hợp, summary rollup, hoặc các phép cộng nội bộ phải chống lỗi khi source day bị thiếu. Quy ước an toàn là: missing source row không được làm crash report; ở các lớp tính toán nội bộ có thể normalize missing official-energy value về \texttt{0}, còn ở lớp hiển thị vẫn nên giữ khả năng phân biệt giữa zero thực và giá trị thiếu bằng ký hiệu như \texttt{-} khi phù hợp.
...
\item \texttt{\_build\_energy\_object()} dùng area energy repositories cộng official-total inputs từ \texttt{total\_energy};
...
\begin{table}[htbp]
	...
	\begin{tabularx}{\linewidth}{|>{\RaggedRight\arraybackslash}p{2.1cm}|>{\RaggedRight\arraybackslash}p{2.6cm}|>{\RaggedRight\arraybackslash}p{2.8cm}|>{\RaggedRight\arraybackslash}X|}
		...
		Electricity & \texttt{total\_energy} + area energy views & \shortstack[l]{\texttt{EnergyDataRepository} \\ $\rightarrow$ \texttt{EnergyService}} & electricity summary cards, top meter tables, daily detail tables, comparison blocks \\ \hline
