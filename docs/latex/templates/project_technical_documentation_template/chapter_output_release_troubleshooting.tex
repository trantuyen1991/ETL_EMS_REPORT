...
\item Ghi nhận release workflow và các manual smoke checks đang được chấp nhận.
...
Chapter này được tổng hợp từ workflow thực tế của dự án và nên được giữ đồng bộ với các tài liệu nguồn như \texttt{docs/workflows/release\_runbook.md}, \texttt{docs/workflows/release\_checklist.md}, và \texttt{docs/workflows/pdf\_export\_flow.md}.
...
\begin{table}[h]
	...
	\begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{2.6cm} >{\raggedright\arraybackslash}p{3.2cm} >{\raggedright\arraybackslash}X}
		\toprule
		\textbf{Artifact Family} & \textbf{Location} & \textbf{Purpose} \\
		\midrule
		View HTML & \texttt{output/reports/YYYY\_MM/view\_html/} & Browser-readable version dùng để đọc report, kiểm tra cấu trúc, và debug render output. \\
		PDF source HTML & \texttt{output/reports/YYYY\_MM/pdf\_source\_html/} & HTML nguồn cho PDF export, phản ánh print-safe shell và compact print styling trước khi in. \\
		Final PDF & \texttt{output/reports/YYYY\_MM/pdf/} & PDF A4 cuối cùng, dùng cho phát hành và lưu trữ. \\
		Daily Excel workbook & \texttt{output/reports/YYYY\_MM/excel/} & Workbook daily-only của v1 Excel export, phục vụ tabular extraction. \\
		\bottomrule
	\end{tabularx}
\end{table}
...
\begin{table}[h]
	...
	\begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{2.7cm} >{\raggedright\arraybackslash}p{2.6cm} >{\raggedright\arraybackslash}X}
		\toprule
		\textbf{Scenario} & \textbf{Expected Periods} & \textbf{Expected Canonical Outputs} \\
		\midrule
		Daily-only anchor & Daily & view HTML, PDF source HTML, final PDF, và daily Excel workbook dưới prefix \texttt{03\_daily}. \\
		Sunday anchor & Daily + Weekly & toàn bộ artifact daily cộng thêm bộ weekly dưới prefix \texttt{02\_weekly}; không có monthly. \\
		Month-end anchor & Daily + Monthly & toàn bộ artifact daily cộng thêm bộ monthly dưới prefix \texttt{01\_monthly}; không có weekly. \\
		Sunday + month-end & Daily + Weekly + Monthly & runtime xuất đủ cả ba period families trong cùng tháng output tương ứng. \\
		\bottomrule
	\end{tabularx}
\end{table}
...
\begin{table}[h]
	...
	\begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{3.3cm} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X}
		\toprule
		\textbf{Symptom} & \textbf{Likely Cause} & \textbf{First Checks} \\
		\midrule
		PDF bị co nhỏ hoặc sai scale & document width overflow, wide periodic tables, hoặc print path không dùng baseline ổn định & kiểm tra table overflow, chart container width, và xác nhận CDP path với \texttt{scale=1.0}, \texttt{preferCSSPageSize=true}. \\
		PDF thiếu chart & chart lifecycle trong PDF mode không được kick off hoặc chart không được freeze thành static SVG trước print & kiểm tra PDF chart init flow, xác nhận \texttt{renderer: \"svg\"}, \texttt{animation: false}, và kickoff sau \texttt{window.load}. \\
		Artifact không xuất hiện ở canonical month folder & copy-back từ staging path thất bại hoặc export path resolve sai & kiểm tra \texttt{output/reports/YYYY\_MM/}, staging directory, và logic copy từ staged PDF về canonical PDF folder. \\
		PDF source staging bị lỗi trên hidden path & Chromium không truy cập ổn staging directory vì path hidden & xác nhận \texttt{PRINT\_STAGING\_DIR} hoặc \texttt{OUTPUT\_DIR} đang trỏ tới non-hidden path. \\
		Excel xuất sai scope & logic daily-only của Excel export bị thay đổi hoặc anchor/scheduling bị hiểu sai & xác nhận Excel chỉ được tạo cho \textit{daily}, không tạo cho weekly/monthly. \\
		Tên file sai prefix hoặc sai base name & config naming drift hoặc helper naming rules bị sửa lệch & kiểm tra \texttt{REPORT\_FILENAME}, các prefix \texttt{01/02/03}, và output examples trong runbook/checklist. \\
		Docs kỹ thuật và runtime không còn khớp nhau & docs chưa được cập nhật sau khi implementation đổi & rà lại \texttt{project\_status}, runbook, checklist, và technical PDF chapter tương ứng. \\
		\bottomrule
	\end{tabularx}
\end{table}
