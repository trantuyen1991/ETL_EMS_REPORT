...
Để deployment gần với kiểu operator docs chỉ cần copy-paste, runbook hiện đã có một block terminal duy nhất để ghi \texttt{config/.env}. Người cài chỉ cần thay các giá trị host-specific như MySQL credentials, output path, staging path, và khi cần thì đổi \texttt{WORKSHOP\_NAME} / \texttt{ENERGY\_UNIT}, sau đó paste một lần vào terminal.
...
Repo hiện đã có ba artifact hỗ trợ triển khai nhanh:
...
Với host bám đúng baseline khuyến nghị, helper script này có thể backup unit cũ nếu có, copy sample files sang \texttt{/etc/systemd/system/}, reload \texttt{systemd}, rồi enable timer trong một lần chạy.
...
Repo sample timer vẫn dùng trực tiếp được nếu host bám theo mốc triển khai khuyến nghị của dự án.
