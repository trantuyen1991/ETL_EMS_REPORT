...
\begin{enumerate}[label=-]
\item clone repository vào một project path ổn định, ví dụ \texttt{/srv/energy-report};
\item tạo lại \texttt{venv} mới ngay trên host đó và cài \texttt{requirements.txt};
...
Service chuẩn nên có các thành phần chính sau:

\begin{enumerate}[label=-]
\item unit file: \texttt{/etc/systemd/system/energy-report-etl.service};
\item \texttt{Type=oneshot};
\item \texttt{User=<linux-user>} và \texttt{Group=<linux-user>};
\item \texttt{WorkingDirectory=<project-root>};
\item \texttt{ExecStart=<project-root>/venv/bin/python -m src.main};
...
\begin{enumerate}[label=-]
\item unit file: \texttt{/etc/systemd/system/energy-report-etl.timer};
