...
\begin{table}[h]
	...
	\begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{3.5cm} >{\raggedright\arraybackslash}X}
		...
		Current document version & \texttt{v0.6-draft} \\
		Document status & Draft review baseline \\
		Primary owner / author & Tuyen Tran \\
		Reviewer & TBD \\
		Approval status & Pending review and sign-off \\
...
Ở trạng thái hiện tại, owner đã rõ, nhưng reviewer và approver vẫn nên được chốt sau khi tài liệu đi đến một mức ổn định hơn.
...
\item reviewer và approval path đã được xác định hoặc ít nhất được để ở trạng thái TBD có chủ đích;
