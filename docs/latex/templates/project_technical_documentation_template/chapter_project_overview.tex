...
Hình \ref{fig:system_architecture} mô tả các lớp chính của hệ thống, bao gồm runtime/configuration inputs, data ownership, repository layer, service layer, report context builder, rendering path, Excel export path, và canonical output structure. Hình \ref{fig:report_generation_flow} mô tả chu trình thực thi từ bước load config, resolve scheduled periods, fetch source rows, aggregate sensor statistics, build domain objects, assemble \texttt{report\_context}, render artifacts, đến lưu output theo cấu trúc tháng.
...
Chapter mẫu này chuyển template từ phong cách học thuật tổng quát sang phong cách technical manual cho dự án báo cáo năng lượng. Ở trạng thái hiện tại, bộ tài liệu đã có chapter metadata / revision history, chapter tổng quan, chapter data sources / mapping, chapter business rules / KPI rules, và chapter output / release operations. Các bước mở rộng tiếp theo có thể tiếp tục bổ sung glossary hoặc approval-oriented appendices khi cần.
