...
\begin{table}[h]
	...
	\begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{3.1cm} >{\raggedright\arraybackslash}p{3.2cm} >{\raggedright\arraybackslash}X}
		\toprule
		\textbf{Domain} & \textbf{Primary Source / Owner} & \textbf{Critical Rule} \\
		\midrule
		Electricity official totals & \texttt{energy\_kpi} & Plant totals và area official totals phải dùng giá trị đã được phê duyệt, không tự cộng lại từ raw energy views. \\
		KPI values and production-linked reporting & \texttt{energy\_kpi} & KPI reporting chỉ dùng các block đã tính sẵn và đã được lưu, tuân thủ coverage-first logic. \\
		Utility daily usage & utility usage source & Utility summary và daily detail phải so sánh current/previous và giữ dense rows cho khoảng ngày cần hiển thị. \\
		Sensor monitoring & \texttt{processvalue} + sensor metadata mapping & Raw sensor rows phải được aggregate thành payload có chất lượng dữ liệu rõ ràng, hỗ trợ anomaly-ready analysis. \\
		Presentation rules & backend services + render context & Template chỉ nhận dữ liệu đã chuẩn hóa từ \texttt{report\_context}; không tái tính nghiệp vụ trong view/PDF. \\
		\bottomrule
	\end{tabularx}
\end{table}
...
\begin{table}[h]
	...
	\begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{2.2cm} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X}
		\toprule
		\textbf{Block Type} & \textbf{Accepted When} & \textbf{Reject When} \\
		\midrule
		Day & row khớp chính xác một ngày cần hiển thị & row trùng nhưng có \texttt{dt\_lastupdate} cũ hơn block mới hơn \\
		Week & block tuần nằm trọn trong uncovered range và không chồng lên các ngày đã có Day coverage hợp lệ & block chỉ overlap một phần hoặc cần bị chia nhỏ để dùng \\
		Month & block tháng được dùng như full stored block hợp lệ cho kỳ tháng tương ứng & block tháng chỉ fit một phần custom range hoặc cần prorating \\
		Year & block năm chỉ dùng như full stored block khi thật sự là kỳ hợp lệ tương ứng & block năm làm override coverage chi tiết hơn hoặc cần phân rã thành smaller units \\
		\bottomrule
	\end{tabularx}
\end{table}
...
Chapter này xác định rõ lớp rules cốt lõi của dự án: source-of-truth discipline, electricity rules, utility/sensor monitoring rules, KPI coverage rules, và các guardrail kỹ thuật để hệ thống luôn cho ra report đúng và có thể bảo trì. Các chapter tiếp theo có thể mở rộng sang output structure, release workflow, troubleshooting, và release-readiness checklist.
