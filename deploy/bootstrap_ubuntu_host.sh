...
run_db_schema_preflight() {
  log "Checking required MySQL tables/views"
...
required_objects = {
    "all_energy_extend": "VIEW",
    "diode_energy_extend": "VIEW",
    "ico_energy_extend": "VIEW",
    "sakari_energy": "VIEW",
    "total_energy": "VIEW",
    "utility_usage": "VIEW",
    "energy_kpi": "BASE TABLE",
    "workshop_timeline": "BASE TABLE",
    "processvalue": "BASE TABLE",
}

required_columns = {
    "all_energy_extend": {"dt"},
    "diode_energy_extend": {"dt"},
    "ico_energy_extend": {"dt"},
    "sakari_energy": {"dt"},
    "total_energy": {"dt"},
    "utility_usage": {"dt"},
    "energy_kpi": {
...
def fail(message: str) -> None:
    print(f"DB schema preflight failed: {message}", file=sys.stderr)
    raise SystemExit(1)
...
try:
    ...
    cursor.execute(
        ...
        WHERE TABLE_SCHEMA = %s
          AND TABLE_NAME IN ({})
        """.format(",".join(["%s"] * len(required_objects))),
        (database, *required_objects.keys()),
    )
    object_types = {row["TABLE_NAME"]: row["TABLE_TYPE"] for row in cursor.fetchall()}

    missing_objects = sorted(set(required_objects) - set(object_types))
    if missing_objects:
        fail("missing required object(s): " + ", ".join(missing_objects))

    wrong_types = sorted(
        name
        for name, expected_type in required_objects.items()
        if object_types.get(name) != expected_type
    )
    if wrong_types:
        details = ", ".join(
            f"{name} expected {required_objects[name]} got {object_types.get(name)}"
            for name in wrong_types
        )
        fail("unexpected object type(s): " + details)
    ...
    cursor.execute(
        ...
        WHERE TABLE_SCHEMA = %s
          AND TABLE_NAME IN ({})
        """.format(",".join(["%s"] * len(required_columns))),
        (database, *required_columns.keys()),
    )
    columns_by_object: dict[str, set[str]] = {}
    for row in cursor.fetchall():
        columns_by_object.setdefault(row["TABLE_NAME"], set()).add(row["COLUMN_NAME"])

    missing_columns: list[str] = []
    for object_name, expected_columns in required_columns.items():
        actual_columns = columns_by_object.get(object_name, set())
        for column_name in sorted(expected_columns - actual_columns):
            missing_columns.append(f"{object_name}.{column_name}")

    if missing_columns:
        fail("missing required column(s): " + ", ".join(missing_columns))

    print(
        "DB schema preflight OK: required views/tables and columns are present.",
        flush=True,
    )
finally:
    conn.close()
